...
| ----------------- | ----------- | ---------------------------------------------------------------------------------------------- |
| **NONE / UNUSED** | `0x00`      | Channel not assigned to any physical sensor. Data (if present) must be ignored.                |
| **FSR**           | `0x01`      | Force Sensitive Resistor (pressure / force sensing channel).                                   |
| **LOADCELL**      | `0x02`      | Load cell sensor channel (strain gauge–based force measurement).                               |
| **IMU**           | `0x03`      | Inertial Measurement Unit channel (accelerometer / gyroscope / magnetometer data).             |
| **RESERVED**      | `0x04–0xFF` | Reserved for future sensor types. Must be ignored by receivers that do not recognize the role. |
...
| `0x09` | STOP_CALIBRATE | Stop calibration |
| `0x10` | END_CALIBRATE | End calibration |
...
| `0x05` | I2C_ERROR | I2C bus failed to connect. |
